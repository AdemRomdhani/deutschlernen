...
export default function HörenExam({ level, onBack, onComplete }) {
  ...
  const renderTeil1 = () => {
    ...
    return (
      <div className="osd-question">
        <button className="btn-play-audio" onClick={() => speak(item.audio)} disabled={isPlaying}>
          {isPlaying ? "⏳ جاري التشغيل..." : "🔊 استمع"}
        </button>
        <div className="osd-audio-text">{item.audio}</div>
        <div className="osd-audio-ar">{item.ar}</div>
  ...
  const renderTeil2 = () => {
    ...
    return (
      <div className="osd-question">
        <div className="osd-passage-ar">{passage.textAr}</div>
  ...
  const renderTeil4 = () => {
    ...
    return (
      <div className="osd-question">
        ...
        <div className="osd-audio-text">{item.audio}</div>
        <div className="osd-audio-ar">{item.ar}</div>
        <div className="osd-statement">{item.statement}</div>
        <div className="osd-statement-ar">{item.statementAr}</div>
        <div className="osd-options truefalse">
          <button
            className={`osd-option ${answers[`4-${currentQ}`] === true ? "selected" : ""}`}
            onClick={() => handleAnswer(4, currentQ, true)}
          >
            ✅ صحيح — Richtig
          </button>
          <button
            className={`osd-option ${answers[`4-${currentQ}`] === false ? "selected" : ""}`}
            onClick={() => handleAnswer(4, currentQ, false)}
          >
            ❌ خطأ — Falsch
          </button>
        </div>
